...
This environment variable can be set to a triplet name which will be used for unqualified host port references in command
lines and all host port references in dependency lists. See [the host-dependencies documentation](host-dependencies.md)
...
This environment variables limits the amount of concurrency used by underlying buildsystems. If unspecified, this defaults
to logical cores + 1.
...
This environment variable adds additional [overlay ports or overlay port directories](../concepts/overlay-ports.md)
...
This environment variable allows using NuGet's cache for every nuget-based binary source.
See [Binary Caching](../reference/binarycaching.md#nuget) for more details.
